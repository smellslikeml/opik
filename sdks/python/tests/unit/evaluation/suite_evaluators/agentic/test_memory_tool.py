...
from opik.evaluation.suite_evaluators.agentic.tools.memory import MemoryTool

# MemoryTool never reads `ctx`; the loop always passes a TraceToolContext,
...
class TestCrossTurnPersistence:
    ...
    def test_memory__fresh_instance_starts_empty(self):
        ...
        # builds a new MemoryTool, so the notepad is scoped to one loop run.
        first = MemoryTool()
...
class TestArgumentValidation:
    def test_memory__missing_action__returns_error(self):
        assert "error" in _run(MemoryTool(), json.dumps({}))

    def test_memory__unsupported_action__lists_supported(self):
        result = _run(MemoryTool(), json.dumps({"action": "prepend"}))
    ...
    def test_memory__invalid_json__returns_error_envelope_not_raise(self):
        ...
        result = json.loads(MemoryTool().execute("not json", _CTX))
        assert "error" in result
...
class TestDefaultRegistryIntegration:
    ...
    def test_default_registry__memory_roundtrip_through_registry(self):
        ...
        # Same registry instance ⇒ same MemoryTool ⇒ same notepad, so the
